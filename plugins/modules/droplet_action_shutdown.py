...
DOCUMENTATION = r"""
---
module: droplet_action_shutdown

short_description: Shutdown a Droplet
...
description:
  - Shutdown a Droplet.
  - |
    A shutdown action is an attempt to shutdown the Droplet in a graceful way, similar to using the shutdown command
    from the console. Since a shutdown command can fail, this action guarantees that the command is issued, not that
    it succeeds.
  - |
    The preferred way to turn off a Droplet is to attempt a shutdown, with a reasonable timeout, followed by a
...
EXAMPLES = r"""
- name: Shutdown a Droplet
  digitalocean.cloud.droplet_action_shutdown:
    token: "{{ token }}"
    state: present
    id: 1122334455
"""
...
class DropletActionShutdown(DigitalOceanCommonModule):
